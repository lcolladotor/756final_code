...
## Example usage:
# Rscript geeInd.R  -p 'derHippo' -d '/Users/lcollado/enigma2/756final_code/results' -c '22' -r 'geeInd' -v TRUE
...
codedir <- getwd()
setwd(reportdir)
knit_bootstrap(file.path(codedir, "geeInd.Rmd"), output=file.path(reportdir, "geeInd.html"), code_style='Brown Paper', chooser=c('boot', 'code'), show_code=TRUE) # show_code=FALSE is specially useful for results with interpretation
